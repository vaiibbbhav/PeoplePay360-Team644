        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input
              type="text"
              value={percentOfCode}
              onChange={(e) => setPercentOfCode(e.target.value.toUpperCase())}
      ...
      <div className="max-w-6xl mx-auto w-full px-4 sm:px-8 py-6 sm:py-8 font-sans">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-3">
            <p className="text-[11px] font-semibold text-ink-soft uppercase tracking-widest px-1">
              Structures
            </p>
              ...
              structures.map((s) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded-full border font-medium shrink-0 ${s.isActive ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-bg-raised text-ink-soft border-line'}`}
                    >
                  ...
                  {s.description && (
                    <div className="text-[11px] text-ink-soft mt-1.5 line-clamp-2">
                      {s.description}
                    </div>
                  )}
                </button>
              ...
              <div className="h-full min-h-[200px] flex items-center justify-center rounded-2xl border border-dashed border-line text-center p-8">
                <div>
                  <p className="text-sm font-medium text-ink-soft">
                    Select a structure to view its rules
                  </p>
                  <p className="text-xs text-ink-soft/60 mt-1">
                    or create a new structure from the panel on the left
                  </p>
                </div>
              </div>
            ) : (
                      ...
                      <>
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            <p className="text-[11px] font-semibold text-ink-soft uppercase tracking-widest">
                              ...
                              <table className="w-full text-xs">
                                <thead>
                                  <tr className="bg-bg-raised border-b border-line">
                                    <th className="text-left px-4 py-2.5 text-ink-soft font-semibold w-10">
                                      #
                                    </th>
                                    <th className="text-left px-4 py-2.5 text-ink-soft font-semibold">
                                      Name
                                    </th>
                                    <th className="text-left px-4 py-2.5 text-ink-soft font-semibold hidden sm:table-cell">
                                      Code
                                    </th>
                                    <th className="text-left px-4 py-2.5 text-ink-soft font-semibold">
                                      Category
                                    </th>
                                    <th className="text-right px-4 py-2.5 text-ink-soft font-semibold">
                                      Method / Amount
                                    </th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {sortedRules.map((rule, idx) => (
                                    <tr
                                      key={rule.id}
                                      className={`border-b border-line last:border-0 ${idx % 2 === 0 ? 'bg-bg' : 'bg-bg-raised/40'}`}
                                    >
                                      ...
                                      <td className="px-4 py-3 font-mono text-ink-soft hidden sm:table-cell">
                                        {rule.code}
                                      </td>
                                      <td className="px-4 py-3">
                                        <span
                                          className={`px-2 py-0.5 rounded-full border text-[10px] font-medium capitalize ${CATEGORY_STYLES[rule.category] ?? CATEGORY_STYLES.other}`}
                                        >
                                          {rule.category}
                                        </span>
                                      </td>
