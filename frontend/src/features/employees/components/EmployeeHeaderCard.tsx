        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 -mt-12 mb-4">
          <div className="flex items-end gap-4">
            ...
            <div className="w-24 h-24 rounded-2xl border-4 border-bg bg-accent-soft text-accent font-serif text-2xl font-bold flex items-center justify-center relative shadow-xs overflow-hidden shrink-0">
              {employee.avatar_url ? (
                <img src={employee.avatar_url} alt={fullName} className="w-full h-full object-cover" />
              ) : (
                <span>{initials}</span>
              )}
            </div>
        ...
        <div className="pt-4 border-t border-line flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-6 text-xs text-ink-soft flex-wrap">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span>Manager: <strong className="font-medium text-ink">{employee.manager_name || 'None (Direct Head)'}</strong></span>
            </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="px-3 py-1 rounded-lg border border-line bg-bg-raised text-[11px] text-ink-soft">
              Contracts: <strong className="text-ink font-semibold">{employee.smartCounts?.contracts ?? 0}</strong>
            </div>
            <div className="px-3 py-1 rounded-lg border border-line bg-bg-raised text-[11px] text-ink-soft">
              Attendance: <strong className="text-ink font-semibold">{employee.smartCounts?.attendance ?? 0}</strong>
            </div>
            <div className="px-3 py-1 rounded-lg border border-line bg-bg-raised text-[11px] text-ink-soft">
              Leaves: <strong className="text-ink font-semibold">{employee.smartCounts?.timeOff ?? 0}</strong>
            </div>
            <div className="px-3 py-1 rounded-lg border border-line bg-bg-raised text-[11px] text-ink-soft">
              Payslips: <strong className="text-ink font-semibold">{employee.smartCounts?.payslips ?? 0}</strong>
            </div>
          </div>
        </div>
      </div>
