...
export const AppLayout: React.FC<AppLayoutProps> = ({ children, title }) => {
  ...
  return (
    <div className="min-h-screen flex bg-bg text-ink font-sans w-full min-w-0 overflow-x-hidden">
      ...
      {/* -------- SIDEBAR -------- */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 border-r border-line bg-bg-raised/40 backdrop-blur-md flex flex-col ${
          sidebarCollapsed ? 'w-16' : 'w-64'
        } ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
      >
        {/* Brand — click goes to landing page */}
        <div
          className={`h-16 px-4 border-b border-line flex items-center ${sidebarCollapsed ? 'justify-center' : 'justify-between'}`}
        >
